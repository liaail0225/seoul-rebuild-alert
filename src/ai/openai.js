...
export async function summarizeArticlesBatch(articles) {
  ...
  return guardedChat({
    ...
    messages: [
      ...
      { role: 'user', content: `아래는 제목에 우리가 추적하는 재건축 사업장 이름이 들어간 기사들이다. 다만 제목에 단지명이 있어도 실제 내용은 그 단지의 재건축 진행과 무관한 경우(인테리어·시공 후기, 다른 단지 비교 언급, 단순 시세 소식 등)가 섞여 있다.

**실제로 그 사업장의 재건축 진행에 관한 신호(설립·인가·총회·심의·시공사선정·이주·철거·분양 등)가 있는 기사만 골라 요약해라. 진행 신호가 없는 기사는 결과에서 완전히 제외하고 언급조차 하지 마라 — "잡음" 이라고 표시하며 나열하지 말고, 아예 빼라.**

형식: "• [자치구 단지명] 핵심 내용". 설립·인가·총회·심의처럼 중요한 것을 먼저. 같은 단지 중복 내용은 합쳐서 한 줄로. 10줄 이내.
남는 기사가 하나도 없으면 정확히 "오늘은 진행 신호가 있는 뉴스가 없습니다." 라고만 답해라.
